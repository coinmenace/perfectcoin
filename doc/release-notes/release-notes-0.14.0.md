...
Commands sent over the JSON-RPC interface and through the `perfectcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`perfectcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/perfectcoin-cli -named help command="help"
    src/perfectcoin-cli -named getblockhash height=0
    src/perfectcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/perfectcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
- #8722 `bb843ad` perfectcoin-cli: More detailed error reporting (laanwj)
...
- #8813 `fb24d7e` perfectcoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow perfectcoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` perfectcoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to perfectcoin-tx (jnewbery)
