...
$ valgrind -v --leak-check=full src/perfectcoind -printtoconsole
```
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `PERFECTCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef PERFECTCOIN_FOO_BAR_H
#define PERFECTCOIN_FOO_BAR_H
...
#endif // PERFECTCOIN_FOO_BAR_H
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `perfectcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `perfectcoin-cli` and the GUI debug console use this table to determine how to
