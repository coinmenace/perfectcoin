 ...
 * between perfectcoind, and bitcoin-qt and non-server tools.
*/
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", PERFECTCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/perfectcoin/perfectcoin>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if perfectcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if perfectcoin is started while in a temporary directory.\n",
