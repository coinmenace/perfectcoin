...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/perfectcoind.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/perfectcoin-cli.1
...
# Source31 - what about perfectcoin-tx and bench_bitcoin ???
...
The perfectcoin-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the perfectcoin-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/perfectcoind %{buildroot}%{_sbindir}/perfectcoind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/perfectcoind 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/perfectcoind/perfectcoind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/perfectcoind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/perfectcoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/perfectcoin-cli.1
%if %{_buildqt}
...
%attr(0755,root,root) %{_sbindir}/perfectcoind
...
%attr(0644,root,root) %{_mandir}/man1/perfectcoind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/perfectcoin-cli
%attr(0755,root,root) %{_bindir}/perfectcoin-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/perfectcoin-cli.1*
